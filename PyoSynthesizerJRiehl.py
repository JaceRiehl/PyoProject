...
def play():
	...
	s = Server().boot()
	# s.start()
	#I tried this to record my audio but it didn't work, the audio never plays so I Recorded it another way right before the gui
	# s.recordOptions(dur=60, filename="./testFile.wav", fileformat=0, sampletype=0, quality=0.9)
	#This is another example of a trigger but I couldn't figure out how to make it work with a sequence.
	...
	a.ctrl()
	#I tried FastSine but it distorts my audio and amplifies it quite a bit, so I didn't end up adding it to the Selector
	...
	#it should be f and f2 in the selector but i went with the lfo to represent it
	...
	#Record
	rec = Record(sel, filename="./JRiehlPyo.wav",chnls=2, fileformat=0, sampletype=2)
	clean = Clean_objects(75, rec)
	clean.start()
